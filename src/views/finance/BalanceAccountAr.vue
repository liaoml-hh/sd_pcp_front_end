  ...
  <div style="margin-left:10px">
    <el-form :inline="true" :model="searchForm" size="mini">
      ...
      <el-form-item label="对账单号">
        <el-input v-model="searchForm.balanceAccountArNo" />
      </el-form-item>
    ...
    >
      ...
      <el-table-column label="客户" prop="customerVO.name" width="220" align="center" />
      <el-table-column label="对账单号" prop="balanceAccountArNo" width="220" align="center" />
    ...
    <el-dialog v-dialogDrag :before-close="closePickupDialog" title="数据对账" top="5vh" :visible.sync="checkVisible" width="80%">
      <balanceAccountArData ref="balanceAccountArData" @parentCancelHandel="parentCancelHandel" />
    </el-dialog>
...
import balanceAccountArData from './detail/balanceAccountArData'
...
export default {
  ...
  methods: {
    ...
    editButton(row) {
      ...
      this.$nextTick(() => {
        this.$refs.balanceAccountArData.getEditData(row)
      })
    },
    /**
     *  删除按钮
    ...
    deleteButton(row) {
      this.$confirm('此操作将永久删除该账单，是否继续？', '提示', {
        ...
        BalanceArApi.delete({ id: row.id, version: row.version }).then(res => {
          if (res) {
    ...
    checkButton() {
      this.checkVisible = true
      this.$nextTick(() => {
        this.$refs.balanceAccountArData.init()
      })
    },
    CheckCancelButton() {
      this.checkVisible = false
