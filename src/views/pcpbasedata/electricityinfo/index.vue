<template>
  ...
  <el-container>
    ...
    <el-main>
      ...
      >
        <el-form ref="formName" :model="cabinetVO" label-width="100px" :rules="rules">
          <el-row>
            <el-col :span="12">
              <el-form-item label="柜号" prop="code" label-width="100px">
                <el-input v-model="cabinetVO.code" placeholder="" @blur="testcode" />
              </el-form-item>
            </el-col>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="最大电池量" label-width="100px">
                <el-input v-model="cabinetVO.maxQuantity" placeholder="" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="当前剩余电池" label-width="100px">
                <el-input v-model="cabinetVO.currentQuantity" placeholder="" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="12">
              ...
              <el-form-item label="电池类型" prop="dictionaryTypeId" label-width="100px">
                <el-select
                  v-model="cabinetVO.batteryType"
                  clearable
                  placeholder="电池类型"
                  style="width:100%"
                >
...
export default {
  ...
  methods: {
    init() {
      commonQueryDictionary({ pageSize: 1000 }).then(res => {
        this.dictionarytypelist = res.data
      })
      commonQueryStation({ pageSize: 1000 }).then(res => {
        this.stationInfoList = res.data
      })
    },
    initStation() {
    ...
    testcode() {
      if (this.cabinetVO.code.length !== 0) {
        commonQuerycabinet({ code: this.cabinetVO.code }).then(res => {
        ...
        (valid) => {
          if (valid) {
            if (this.dialogType === 'ADD') {
              const cabinetDTO = this.cabinetVO
              addcabinet(cabinetDTO).then(response => {
                if (response) {
              ...
              const updateauditdto = this.cabinetVO
              updatecabinet(updateauditdto).then(response => {
                if (response) {
                  this.$message.success('操作成功')
                  this.dialogVisible = false
                  this.searchFormclick()
    ...
    searchFormclick() {
      ...
      commonQuerycabinet(this.searchForm).then(response => {
    ...
    batchUpdateAuditStatus(arry) {
      batchUpdatecabinet(arry).then(res => {
        if (res) {
    ...
    deleteSelect() {
      this.$confirm('确定要删除吗？').then(res => {
        if (res === 'confirm') {
          batchRemovecabinet(this.$refs.muiltitable.selection).then(res => {
            if (res) {
