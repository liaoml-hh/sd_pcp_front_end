    ...
    <el-main>
      ...
      >
        <div style="margin-left:-30px">
          <el-form ref="formName" :model="auditVo" :rules="updataformrules">
            <el-row>
              <el-col :span="12">
                <el-form-item label="编号" prop="code" label-width="100px">
                  <el-input v-model="auditVo.code" placeholder="" :disabled="true" @blur="testcode" />
                </el-form-item>
              </el-col>
              <el-col :span="12">
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item label="默认组" label-width="100px" prop="defaultFlag">
                  <el-input v-model="auditVo.defaultFlag" placeholder="" />
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="规则" prop="ruleId" label-width="100px">
                  <el-input v-model="auditVo.ruleId" placeholder="" />
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
...
import {
  commonQueryAuditgroup,
  addAuditgroup,
  updateAuditgroup,
  batchremoveAuditgroup,
  removeByIdAuditgroup,
  batchupadateAuditgroup,
  queryByIdAuditgroup
} from '../../../api/auditgroup/index'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      updataformrules: {
        ...
        defaultFlag: [
          { required: true, message: '请输入默认组', trigger: 'blur' }
        ]
      }
  ...
  methods: {
    ...
    testcode() {
      ...
      //   commonQueryAuditgroup({ code: this.auditVo.code }).then(res => {
        ...
        (valid) => {
          if (valid) {
            if (this.dialogType === 'ADD') {
              ...
              addAuditgroup(auditDTO).then(response => {
                if (response) {
              ...
              updateAuditgroup(updateauditdto).then(response => {
                if (response) {
                  this.$message.success('操作成功')
                  this.dialogVisible = false
                  this.searchFormclick()
    ...
    deleteHandle(val) {
      this.$confirm('确定要删除吗？').then(res => {
        removeByIdAuditgroup({ id: val.id }).then(response => {
          this.$message.success('操作成功')
          this.searchFormclick()
        })
      })
    ...
    searchFormclick() {
      ...
      commonQueryAuditgroup(this.searchForm).then(response => {
    ...
    batchUpdateAuditStatus(arry) {
      batchupadateAuditgroup(arry).then(res => {
        if (res) {
    ...
    deleteSelect() {
      this.$confirm('确定要删除吗？').then(res => {
        if (res === 'confirm') {
          batchremoveAuditgroup(this.$refs.muiltitable.selection).then(res => {
            if (res) {
    ...
    rowCopy(row) {
      queryByIdAuditgroup({ id: row.id }).then(res => {
        this.auditVo = res
        this.dialogVisible = true
      }).catch(error => {
        console.log(error)
